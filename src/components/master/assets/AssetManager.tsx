...
import AssetSubOptions from './AssetSubOptions';
import AsstesHeader from './AsstesHeader';
...
const AssetManager = () => {
  ...
  //for getting main-category id
  useEffect(() => {
    const fetchMainId = async () => {
      const resp = await mainCategoryGetApi();
      if (resp && resp.data && resp.data.length > 0) {
        // console.log(resp.data[0].id,'respp')
        const mainCategoryId = resp.data[0].id;
  ...
  // api call for list category and sub-category
  useEffect(() => {
    if (!mainId) return;
  ...
  return (
    <>
      <AsstesHeader openModal={openModal} />
