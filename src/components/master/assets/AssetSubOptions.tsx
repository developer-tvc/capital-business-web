...
interface AssetSubOptionsProps {
  categories: Category[];
  setCategories: React.Dispatch<React.SetStateAction<Category[]>>;
  isModalOpen: boolean;
  setIsModalOpen: React.Dispatch<React.SetStateAction<boolean>>;
}
const AssetSubOptions = ({
  categories,
  setCategories,
  isModalOpen,
  setIsModalOpen
}: AssetSubOptionsProps) => {
  const [newAssetName, setNewAssetName] = useState('');
  const [newGlId, setNewGlId] = useState('');
  // const [localAssets, setLocalAssets] = useState<Subcategory[]>([]);
  ...
  const handleAddAsset = async e => {
    ...
    // Check for existing GL code before making the API call
    //   const existingAssets = categories.flatMap(category =>
    //     category.subcategories ? category.subcategories : []
    // );

    // const isGlCodeDuplicate = existingAssets.some(asset => asset.gl_code === newGlId);

    // if (isGlCodeDuplicate) {
...
export default AssetSubOptions;
