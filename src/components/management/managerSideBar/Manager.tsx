...
import { deleteManagerApi, listMangerAPI } from '../../../api/userServices';
import threeDots from '../../../assets/svg/threeDots.svg';
import { MangerTableHead } from '../../../utils/data';
...
import MangerHeader from './ManagerHeader';
...
const Manager = () => {
  ...
  const handleSortColumn = column => {
    if (column === 'id') {
      ...
      setIsLoading(true);
      handleSort('manager_id', customerIdSortOrder);
    }
  };
  ...
  const { deleteItem } = useDeleteApi(deleteManagerApi); // Use the custom hook

  const handleDelete = async () => {
    const result = await deleteItem(managerId);
    if (result.success) {
  ...
  return (
    <>
      <MangerHeader
        ...
        fetchManagerDetails={fetchManagerAdded}
      />
      {isLoading && (
        <div
          aria-hidden="true"
          ...
          {(isLaptop || isTablet) && (
            <table className="min-w-full divide-y divide-gray-200 overflow-x-auto border">
              <thead className="bg-[#D3D3D3]">
                <tr>
                  {MangerTableHead.map(({ name, key }, index) => (
                    // <>
                    <th
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {managers?.length > 0 ? (
                  ...
                  <tr>
                    <td
                      colSpan={MangerTableHead.length}
