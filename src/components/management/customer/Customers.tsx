...
import { deleteCustomerApi, listCustomerApi } from '../../../api/userServices';
import threeDots from '../../../assets/svg/threeDots.svg';
import { authSelector } from '../../../store/auth/userSlice';
import { customerTableHead } from '../../../utils/data';
...
const Customers = () => {
  ...
  } = usePagination(listCustomerApi);

  const { role } = useSelector(authSelector);
  const { deleteItem } = useDeleteApi(deleteCustomerApi); // Use the custom hook
  ...
  const handleDelete = async () => {
    const result = await deleteItem(customerId);
    if (result.success) {
  ...
  return (
    <>
      <Header title="Customers" onSearch={handleSearch} />
      {isLoading && (
        <div
          aria-hidden="true"
              ...
              <table className="min-w-full divide-y divide-gray-200 overflow-x-auto border">
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {customers?.length > 0 ? (
                    ...
                    <tr>
                      <td
                        colSpan={customerTableHead.length}
