...
import { addressProofApproval } from '../../../api/loanServices';
...
const DocumentEditApproval: React.FC<ProfileDetailsProps> = ({
  profile,
  setIsModalOpen
}) => {
  ...
  // Old changes for Address Proof
  const renderAddressProofOld = () => (
    <>
      {renderFileSection(
        'Council Tax',
        'C4B_REP',
  ...
  const handleApprove = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await addressProofApproval(data);
  ...
  // const headerTitle =
  //   documentType === ApprovalType.AddressProof
  //     ? "Address Proof Documents"
  //     : documentType === ApprovalType.PhotoId
  //     ? "Photo Id Document"
  //     : "Document viewer";
  //////
  // const renderUploadSection = (id: string) => (
  //   <div className="w-full md:w-1/3 px-4 py-4">
  //     <FileUploadDropzone id={id} editMode={isEditMode} />
  //   </div>
  // );

  // const hasFiles = councilTaxSize || utilityBillSize || leaseDeedSize;

  return (
    <>
      <Header title="Address Proof Documents" />
...
export default DocumentEditApproval;
