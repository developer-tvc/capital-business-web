...
import { addFinanceManagerApi } from '../../../api/userServices';
import { authSelector } from '../../../store/auth/userSlice';
import {
  loanFormCommonStyleConstant,
  manageManageFinanceManager
} from '../../../utils/constants';
import { Roles } from '../../../utils/enums';
import { NotificationType } from '../../../utils/hooks/toastify/enums';
import useToast from '../../../utils/hooks/toastify/useToast';
import { ManageFinanceManagerSchema } from '../../../utils/Schema';
import { ManageFinanceManagerInterface } from '../../../utils/types';
...
type FinanceManagerHeaderProps = HeaderProps;

const FinanceManagerHeader: React.FC<FinanceManagerHeaderProps> = ({
  title,
  onSearch,
  onFilterChange,
  dropdownData,
  ...
  const fieldRenderer = new FieldRenderer(
    manageManageFinanceManager,
    loanFormCommonStyleConstant,
    ManageFinanceManagerSchema
  );
  const methods = useForm<ManageFinanceManagerInterface>({
    resolver: yupResolver(ManageFinanceManagerSchema)
  });
  ...
  const onSubmit = async (data: ManageFinanceManagerInterface) => {
    try {
      setIsLoading(true);
      const response = await addFinanceManagerApi(data);
      ...
      console.error('Failed to add finance manager:', error);
      showToast('Failed to add finance manager', {
              ...
              <div className="flex justify-end px-4 pt-6">
                ...
                <p className="my-1 pl-2 text-[15px] font-medium">
                  ...
                  <div className="text-[10.5px] text-[#656565]">
                    {'Hey! you can create a Finance Manager from Here!'}
                  </div>
                </p>
                <button
                  onClick={toggleModal}
...
export default FinanceManagerHeader;
