...
import { financeManagerTableHead } from '../../../utils/data';
...
import FiledAgentHeader from './FinanceManagerHeader';
import FiledAgentModal from './FinanceManagerModal';
...
const FinanceManager = () => {
  ...
  } = usePagination(listFinanceManagerApi);
  ...
  const handleSortColumn = column => {
    if (column === 'id') {
      ...
      setIsLoading(true);
      handleSort('finance_manager_id', customerIdSortOrder);
    }
  };

  const closeModal = () => setIsModalOpen(false);
  const closeDeleteModal = () => setIsDeleteModalOpen(false);
  const { deleteItem } = useDeleteApi(deleteFinanceManagerApi); // Use the custom hook
  const handleDelete = async () => {
    const result = await deleteItem(agentId);
    if (result.success) {
      setFinanceManagers(
        financeManagers.filter(financeManager => financeManager.id !== agentId)
      );
      closeDeleteModal();
    }
  };
  ...
  return (
    <>
      <FiledAgentHeader title="Finance Manager" onSearch={handleSearch} />
      {isLoading && (
        <div
          aria-hidden="true"
          ...
          {(isLaptop || isTablet) && (
            <table className="min-w-full divide-y divide-gray-200 overflow-x-auto border">
              <thead className="bg-[#D3D3D3]">
                <tr>
                  {financeManagerTableHead.map(({ name, key }, index) => (
                    <th
                      key={index}
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {financeManagers?.length > 0 ? (
                  ...
                  <tr>
                    <td
                      colSpan={financeManagerTableHead.length}
