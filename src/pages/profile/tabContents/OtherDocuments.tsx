...
import { customerOtherDocumentsApi } from '../../../api/documentsApi';
...
const OtherDocuments: React.FC<{ unitId?: number | string }> = ({ unitId }) => {
  ...
  const fetchFileData = async () => {
    ...
    setIsSelected(true);
    try {
      const customerOtherDocumentsApiResponse =
        await customerOtherDocumentsApi(companyId);
      if (
        customerOtherDocumentsApiResponse.status_code >= 200 &&
        customerOtherDocumentsApiResponse.status_code < 300
      ) {
        const otherFiles = customerOtherDocumentsApiResponse?.data?.other_files;
        if (otherFiles.length > 0) {
          setOtherFilesData(otherFiles);
        }
      } else {
        showToast(customerOtherDocumentsApiResponse.status_message, {
